...
- [x] **[build] Conventional Commits not enforced by CI; no CHANGELOG or SECURITY.md** (`.github/workflows/`)
  ...
  Done: added `.github/workflows/commitlint.yml` (runs on every PR via `wagoid/commitlint-github-action`) and `commitlint.config.js`, restricted to the exact type list in docs/conventions.md. CHANGELOG.md is intentionally omitted by maintainer decision: GitHub Releases (generated by semantic-release from Conventional Commits) are the changelog. SECURITY.md remains an optional future addition.
...
- [x] **[build] commitlint rules reject this branch's own commits** (`commitlint.config.js:13,15`)
