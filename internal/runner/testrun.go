...
func (r *testRun) buildExecConfig() error {
	...
	if r.useDocker {
		r.mockHostIP = docker.OutboundIP()
	}
	...
	r.execCfg = executor.ExecutorConfig{
		...
		DockerVolumes:       docker.VolumeMounts(r.useDocker, r.work.Dir, r.dockerVolumeName, r.suite.volumeStrategy),
		DockerExtraHosts:    docker.ExtraHosts(r.useDocker, r.mockHostIP),
		HostEnv:             r.suite.opts.HostEnv,
